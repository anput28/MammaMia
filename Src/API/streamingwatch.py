...
async def search(showname, season, episode, date, ismovie, client):
    if ismovie == 1:
        ...
        id_response = await client.get(
            f'https://streamingwatch.{SW_DOMAIN}/wp-json/wp/v2/categories?search={showname}&_fields=id',
        ...
        for entry in data:
            if f"stagione-{season}-episodio-{episode}" in entry[
                "slug"] and f"stagione-{season}-episodio-{episode}0" not in entry["slug"]:
